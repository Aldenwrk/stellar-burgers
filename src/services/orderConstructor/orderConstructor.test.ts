...
import { orderConstructorSlice, addItem, removeItem, moveItem, initialState} from "./slice";
...
describe('Проверка редьюсера конструктора', ()=>{
  it('Экшн addItem добавляет ингредиент', ()=>{
    ...
    expect(result.constructorItems.ingredients[0].name).toBe("Биокотлета из марсианской Магнолии");
  ...
  it('Экшн removeItem удаляет ингредиент', ()=>{
    ...
    expect(stateWithIngredient.constructorItems.ingredients[0].name).toBe("Биокотлета из марсианской Магнолии");
  ...
  it('Экшн moveItem перемещает ингредиент в списке', ()=>{
    const stateWithIngredient = constructorReducer(initialState, { type: addItem.type, payload:mockIngredient});
    const stateWithIngredient2 = constructorReducer(stateWithIngredient, { type: addItem.type, payload:anotherMockIngredient});
    expect(stateWithIngredient2.constructorItems.ingredients.length).toBe(2);
    expect(stateWithIngredient2.constructorItems.ingredients[0].name).toBe("Биокотлета из марсианской Магнолии");

    const action = {type: moveItem.type, payload:{ingredient:anotherMockIngredient, direction:'moveDown'}};
    const result = constructorReducer(stateWithIngredient2, action);
    expect(result.constructorItems.ingredients.length).toBe(2);
    expect(result.constructorItems.ingredients[0].name).toBe("Точно не биокотлета из марсианской Магнолии");
  });
